...
void load_ascii()
{
	ascii = LoadSprite("text/some_ascii_red.png", 8192, 32);
}
...
void text_draw2(char* text, Vec2D pos, Vec2D size)
{
	int i;
	int value;
	...
	for (i = 0; i < strlen(text); i++)
	{
		...
		{
			slog("%i", atoi(text[i]));
			drawToTheFrigginScreen(pos, size, ascii->texture, atoi(text[i]), i);
		}
	}
}
